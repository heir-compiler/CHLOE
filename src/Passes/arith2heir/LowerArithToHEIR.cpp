...
public:
        ...
        OpType op, typename OpType::Adaptor adaptor, ConversionPatternRewriter &rewriter) const override
    {
        ...
        {
            // If operand is defined in ConstantOp, its type should be Plain
            if (auto constOp = o.getDefiningOp<ConstantOp>()) {
                auto plainType = PlainType::get(rewriter.getContext());
                auto matOp = rewriter.create<FHEMaterializeOp>(op.getLoc(), plainType, constOp.getResult());
                materialized_operands.push_back(matOp.getResult());
                continue;
            }
        ...
        {
            rewriter.replaceOpWithNewOp<LWESubOp>(op, TypeRange(dstType), 
                materialized_operands, FloatAttr::get(Float64Type::get(rewriter.getContext()), -1.0));
            return success();
        }
        return failure();
    };
...
public:
        ...
        CmpFOp op, typename CmpFOp::Adaptor adaptor, ConversionPatternRewriter &rewriter) const override
    {
        ...
        if (auto constOp = lhs.getDefiningOp<ConstantOp>()) {
            auto plainType = PlainType::get(rewriter.getContext());
            auto matOp = rewriter.create<FHEMaterializeOp>(op.getLoc(), plainType, constOp.getResult());
            new_lhs = matOp.getResult();
        } 
        else if (lhs.getType() != operandDstType) {
        ...
        if (auto constOp = rhs.getDefiningOp<ConstantOp>()) {
            auto plainType = PlainType::get(rewriter.getContext());
            auto matOp = rewriter.create<FHEMaterializeOp>(op.getLoc(), plainType, constOp.getResult());
            new_rhs = matOp.getResult();
        }
        else if (rhs.getType() != operandDstType) {
...
LogicalResult ArithConstOperation(IRRewriter &rewriter, MLIRContext *context, ConstantOp op, TypeConverter typeConverter)
{
    ...
    auto dstType = PlainType::get(rewriter.getContext());
    FloatAttr message = op.getValueAttr().dyn_cast_or_null<FloatAttr>();
    rewriter.setInsertionPoint(op);
    auto encodeOp = rewriter.create<FHEEncodeOp>(op.getLoc(), dstType, message, 
                        FloatAttr::get(Float64Type::get(rewriter.getContext()), -1.0));
    rewriter.replaceOpWithNewOp<FHEMaterializeOp>(op, op.getType(), encodeOp.getResult());
    ...
    //             rewriter.replaceOpWithNewOp<FHEEncodeOp>(mat_op, PlainType::get(rewriter.getContext()), new_message, FloatAttr::get(Float64Type::get(rewriter.getContext()), -1.0));
    //         }
    //     }
    // }
...
void LowerArithToHEIRPass::runOnOperation()
{
    ...
    target.addLegalDialect<AffineDialect, func::FuncDialect, scf::SCFDialect, ArithDialect>();
    ...
    newtarget.addLegalDialect<AffineDialect, func::FuncDialect, scf::SCFDialect, ArithDialect>();
